...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagAll_Led)
    {
        info->event[info->num] = EVENT_All_Led;
        info->num++;
        dataPoints->valueAll_Led = gizStandardDecompressionValue(All_Led_BYTEOFFSET,All_Led_BITOFFSET,All_Led_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagauto_rain)
    {
        info->event[info->num] = EVENT_auto_rain;
        info->num++;
        dataPoints->valueauto_rain = gizStandardDecompressionValue(auto_rain_BYTEOFFSET,auto_rain_BITOFFSET,auto_rain_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagauto_hum_tem)
    {
        info->event[info->num] = EVENT_auto_hum_tem;
        info->num++;
        dataPoints->valueauto_hum_tem = gizStandardDecompressionValue(auto_hum_tem_BYTEOFFSET,auto_hum_tem_BITOFFSET,auto_hum_tem_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flaghandle_humidity_Temp)
    {
        info->event[info->num] = EVENT_handle_humidity_Temp;
        info->num++;
        dataPoints->valuehandle_humidity_Temp = gizStandardDecompressionValue(handle_humidity_Temp_BYTEOFFSET,handle_humidity_Temp_BITOFFSET,handle_humidity_Temp_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagwindow_open_rangle)
    {
        info->event[info->num] = EVENT_window_open_rangle;
        info->num++;
        dataPoints->valuewindow_open_rangle = gizX2Y(window_open_rangle_RATIO,  window_open_rangle_ADDITION, issuedData->attrVals.valuewindow_open_rangle); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueAll_Led != cur->valueAll_Led)
    {
        GIZWITS_LOG("valueAll_Led Changed\n");
        ret = 1;
    }
    if(last->valueauto_rain != cur->valueauto_rain)
    {
        GIZWITS_LOG("valueauto_rain Changed\n");
        ret = 1;
    }
    if(last->valueauto_hum_tem != cur->valueauto_hum_tem)
    {
        GIZWITS_LOG("valueauto_hum_tem Changed\n");
        ret = 1;
    }
    if(last->valuehandle_humidity_Temp != cur->valuehandle_humidity_Temp)
    {
        GIZWITS_LOG("valuehandle_humidity_Temp Changed\n");
        switch(cur->valuehandle_humidity_Temp)
        {
          case handle_humidity_Temp_VALUE0:
            ...
            break;
          case handle_humidity_Temp_VALUE1:
                LCD_SetTextColor(BLUE);
                ILI9341_DisplayStringEx(0,216, 32,32,"I'm increasing!",0);
            break;
          case handle_humidity_Temp_VALUE2:
                LCD_SetTextColor(WHITE);
                ILI9341_DisplayStringEx(0,216, 32,32,"I'm decreasing!",0);
            break;
          default:
            break;
        }
    ...
    if(last->valuewindow_open_rangle != cur->valuewindow_open_rangle)
    {
        GIZWITS_LOG("valuewindow_open_rangle Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(All_Led_BYTEOFFSET,All_Led_BITOFFSET,All_Led_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAll_Led);
    gizStandardCompressValue(auto_rain_BYTEOFFSET,auto_rain_BITOFFSET,auto_rain_LEN,(uint8_t *)devStatusPtr,dataPoints->valueauto_rain);
    gizStandardCompressValue(auto_hum_tem_BYTEOFFSET,auto_hum_tem_BITOFFSET,auto_hum_tem_LEN,(uint8_t *)devStatusPtr,dataPoints->valueauto_hum_tem);
    if(dataPoints->valuehandle_humidity_Temp >= handle_humidity_Temp_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuehandle_humidity_Temp Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(handle_humidity_Temp_BYTEOFFSET,handle_humidity_Temp_BITOFFSET,handle_humidity_Temp_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehandle_humidity_Temp);  
    }
    if(dataPoints->valueRain >= Rain_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueRain Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Rain_BYTEOFFSET,Rain_BITOFFSET,Rain_LEN,(uint8_t *)devStatusPtr,dataPoints->valueRain);  
    }
    ...
    devStatusPtr->valuewindow_open_rangle = gizY2X(window_open_rangle_RATIO,  window_open_rangle_ADDITION, dataPoints->valuewindow_open_rangle); 
